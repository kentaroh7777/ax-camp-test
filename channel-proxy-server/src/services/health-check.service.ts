...
import { HealthCheckResponse, MetricsResponse } from '../types/api.types.js';
...
export class HealthCheckService {
  ...
  async getStartupStatus(): Promise<{ ready: boolean; startupTime: number; checks: any[] }> {
    ...
    const checks = [
      { name: 'configuration', status: this.checkConfiguration() ? 'healthy' : 'unhealthy' },
      { name: 'dependencies', status: await this.checkStartupDependencies() ? 'healthy' : 'unhealthy' },
      { name: 'resources', status: this.checkSystemResources() ? 'healthy' : 'unhealthy' },
    ];

    const ready = this.isReady && checks.every(check => check.status === 'healthy');
  ...
  private async checkCriticalDependencies(): Promise<Array<{ name: string; status: 'healthy' | 'unhealthy' }>> {
    const dependencies: Array<{ name: string; status: 'healthy' | 'unhealthy' }> = [];
