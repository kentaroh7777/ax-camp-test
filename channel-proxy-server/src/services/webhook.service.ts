...
import { ChannelType } from '../types/discord.types.js';
...
export class WebhookService {
  ...
  async verifySignature(body: Buffer | string, signature: string, channelSecret: string): Promise<boolean> {
    // If webhook signature validation is disabled, skip verification
    if (!config.WEBHOOK_SIGNATURE_VALIDATION) {
      logger.info('Webhook signature verification skipped by configuration in WebhookService.');
      return true;
    }
  ...
  private async processMessageEvent(event: LineMessageEvent): Promise<void> {
    ...
    // For setup-wizard: Log the user ID clearly to the console
    if (event.source.type === 'user' && event.source.userId) {
        console.log('\n--------------------------------------------------');
        console.log('>>> LINE User ID Detected for Setup Wizard <<<');
        console.log(`>>> User ID: ${event.source.userId}`);
        console.log('--------------------------------------------------\n');
    }

    // Convert LINE message to unified Message format and add to cache
    const { message, source, timestamp, replyToken } = event;
    
    const unifiedMessage = {
      id: replyToken || `line-${Date.now()}`,
      threadId: source.groupId || source.roomId || source.userId || 'unknown',
      from: source.userId || 'unknown',
      to: 'bot', // LINEボットへの送信
    ...
    messageCache.add(unifiedMessage);
    
    logger.info('LINE message added to cache', {
      messageId: unifiedMessage.id,
      from: unifiedMessage.from,
      contentLength: unifiedMessage.content.length,
  ...
  private extractMessageContent(message: any): string {
    switch (message.type) {
      ...
      case 'location':
        return `[Location: ${message.title || 'unknown'}]`;
      case 'sticker':
        return '[Sticker]';
      default:
